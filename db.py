...
client = MongoClient(
    "mongodb+srv://user_1997:Ix5HFdIrvscXChxc@roomapp.2b4r9.mongodb.net/?retryWrites=true&w=majority")
...
def save_user(username, email, password,image):
    password_hash = generate_password_hash(password)
    users_collection.insert_one(
        {'_id': username, 'email': email, 'password': password_hash,'image':image})


def get_user(username):
    user_data = users_collection.find_one({'_id': username})
    return User(user_data['_id'], user_data['email'], user_data['password'],user_data['image']) if user_data else None
