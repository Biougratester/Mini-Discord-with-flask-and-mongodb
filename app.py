...
def signup():
    ...
    if request.method == 'POST':
        ...
        try:
            save_user(username, email, password, filename)
            return redirect(url_for('login'))
        except DuplicateKeyError:
            message = "User already exists!"
    return render_template('signup.html', message=message,image=filename)
    ...
    if request.method == 'POST':
        ...
        if len(room_name) and len(usernames):
            ...
            add_room_members(room_id, room_name, usernames,
                             current_user.username,current_user.image )
...
def view_room(room_id):
    room = get_room(room_id)
    if room and is_room_member(room_id, current_user.username,current_user.image):
        room_members = get_room_members(room_id)
        messages = get_messages(room_id)
        return render_template('view_room.html', username=current_user.username,image=current_user.image, room=room, room_members=room_members,
                               messages=messages)
    else:
        return "Room not found", 404
